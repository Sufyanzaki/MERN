...
const SidebarRight = ({ handleChat, chatBox, chatFun }) => {
  ...
  const [userId, setUserId] = useState({name : '', id:null});
  const [loading, setLoading] = useState(false);

  const sendMessage = (e, id) => {
    e.preventDefault();
    axios.post(`${baseURI}message`, { userId: id }, { withCredentials: true }).then(res => {
      let chat = res.data.chat;
      chat = Array.isArray(chat) ? chat[0] : chat;
      setChatCollection([chat,...chatCollection])
      setMessage('');
      let targetChat = Array.isArray(res.data.chat) ? res.data.chat[0]._id : res.data.chat._id
      axios.post(`${baseURI}sendmessage`, { content: message, chatId: targetChat }, { withCredentials: true }).then((res) => {
        const { data } = res;
  ...
  useEffect(() => {
    ...
    axios.get(`${baseURI}fetchchats`, { withCredentials: true })
      .then(res => {setChatCollection(res.data); setLoading(false)})
      .catch(err => console.log(err));
  }, [])
  ...
  useEffect(() => {
    ...
    socket.on('profile-pic-server', (data) => setNewPic(data));
  })
    ...
    <div className="fixed-sidebar right">
      <div className="chat-friendz">
        ...
        <div className="chat-users">
          <SimpleBar style={{ minHeight: '38vh' }}>
            <main>
              {allUsers.length > 0 && allUsers.map((u) => {
                return (
                  <div key={u._id} className="author-thmb" title={u.name} onClick={handleChat}>
                    <img src={newPic ? newPic.pic : u.pic} alt="" onClick={()=>{setUserId({name : u.name, id:u._id})}}/>
                    <span className="status f-online"></span>
                  </div>)
              })}
            </main>
          </SimpleBar>
          {!loading ? <div className="author-thmb chat" onClick={() => { showChats(!chats); setSelectedChat(null);}}>
        ...
        {chats && <div className={`chat-box show`}>
          <div className="chat-head">
            ...
            <div className="more">
              ...
              <span className="close-mesage" onClick={() => { showChats(false) }}>
          ...
          <div className="chat-list">
            <SimpleBar style={{ minHeight: 300 }}>
              <ul>
                {chatCollection.length > 0 && chatCollection.map((c) => {
                  console.log(c)
                  return <li key={c._id} onClick={()=>{setUserId({name:'', id:getSenderFull(user, c.users)._id})}}>
        ...
        {(chatBox.active || selectedChat) && <div className={`chat-box show`}>
          <div className="chat-head">
            <span className="status f-online"></span>
            <h6>{userId.name}</h6>
            <div className="more">
              ...
              <span className="close-mesage" onClick={() => { setSelectedChat(null); chatFun({ active: false, class: '' }) }}>
                <svg viewBox="0 0 320 512"><path d="M315.3 411.3c-6.253 6.253-16.37 6.253-22.63 0L160 278.6l-132.7 132.7c-6.253 6.253-16.37 6.253-22.63 0c-6.253-6.253-6.253-16.37 0-22.63L137.4 256L4.69 123.3c-6.253-6.253-6.253-16.37 0-22.63c6.253-6.253 16.37-6.253 22.63 0L160 233.4l132.7-132.7c6.253-6.253 16.37-6.253 22.63 0c6.253 6.253 6.253 16.37 0 22.63L182.6 256l132.7 132.7C321.6 394.9 321.6 405.1 315.3 411.3z" /></svg>
              </span>
            </div>
          </div>
          <div className="chat-list">
            <SimpleBar style={{ maxHeight: 300 }}>
              <ul>
                {selectedChat && msgCollection.map((m) => {
            ...
            <form className="text-box">
              ...
              <textarea value={message} onKeyDown={(e) => (e.code === 'Enter' ? sendMessage(e, userId.id) : null)} onChange={handleChange} placeholder="Post enter to post..."></textarea>
